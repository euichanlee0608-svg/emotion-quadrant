...
// 동작 확인용 텔레그램 알림(COT봇). 실패해도 사용자 응답은 막지 않는다.
async function tell(db: ReturnType<typeof createClient>, lines: string[]) {
  try {
    const { data } = await db.from("app_secrets").select("key,value")
      .in("key", ["telegram_notify_token", "telegram_owner_chat"]);
    const g = (k: string) => data?.find((x: { key: string }) => x.key === k)?.value ?? "";
    const token = g("telegram_notify_token"), chat = g("telegram_owner_chat");
...
const VIA_LABEL: Record<string, string> = {
  local: "로컬 모델(맥미니)", edge: "서버 Gemini", match: "글자 매칭(폴백)",
  queued: "맥미니 대기열", worker: "맥미니 로컬 모델",
};

/* 큐 액션들 — 제미나이가 다 떨어졌을 때 맥미니가 대신 처리하게 하는 통로.
   맥미니는 바깥에서 접속할 수 없으므로(방화벽·https 혼합콘텐츠) **맥미니가 가지러 온다**.
   claim/finish 는 워커 토큰으로만 열린다. */
/* 제미나이를 못 쓰는 상황(한도 소진·전부 실패)에서 맥미니에 넘긴다 */
async function queueJob(db: ReturnType<typeof createClient>, text: string, reason: string) {
  const { data: job } = await db.from("emoquad_jobs").insert({ text }).select("id").single();
  await tell(db, [
    `🫧 감정의 사분면 — 제미나이를 못 써서 맥미니로 넘김`, ``,
    `입력: ${text}`, `사유: ${reason}`,
    job?.id ? `작업: ${job.id}` : `대기열 등록 실패`,
  ]);
  return job?.id ?? null;
}

async function workerOK(db: ReturnType<typeof createClient>, token: unknown) {
  if (!token) return false;
  const { data } = await db.from("app_secrets").select("value").eq("key", "emoquad_worker_token").single();
  return !!data?.value && String(token) === data.value;
}
...
Deno.serve(async (req) => {
  ...
  // 로컬 모델이나 폴백이 처리한 경우 — 알림만 중계한다(모델 호출 없음, 캡도 안 쓴다)
  if (body.action === "notify") {
    const t = String(body.text ?? "").slice(0, 400);
    const ws = Array.isArray(body.words) ? body.words.map(String).filter((w) => ALLOWED.has(w)) : [];
    if (t) {
      await tell(db, [
        `🫧 감정의 사분면 — ${VIA_LABEL[String(body.via)] ?? String(body.via)}`,
        ``, `입력: ${t}`, `추천: ${ws.join(", ") || "(없음)"}`,
      ]);
    }
    return json({ ok: true });
  ...
  // 브라우저가 대기열 작업 결과를 물어본다
  if (body.action === "job") {
    const { data } = await db.from("emoquad_jobs").select("status,read,words,err")
      .eq("id", String(body.id ?? "")).maybeSingle();
    if (!data) return json({ error: "작업을 찾을 수 없습니다" }, 404);
    return json(data);
  }

  // 맥미니 워커가 처리할 작업을 하나 집어 간다
  if (body.action === "claim") {
    if (!(await workerOK(db, body.token))) return json({ error: "권한 없음" }, 401);
    const { data, error } = await db.rpc("emoquad_claim");
  ...
  if (body.action === "finish") {
    if (!(await workerOK(db, body.token))) return json({ error: "권한 없음" }, 401);
    const ws = Array.isArray(body.words)
      ? body.words.map(String).map((w: string) => w.trim()).filter((w: string) => ALLOWED.has(w)).slice(0, 6)
      : [];
    const okRun = ws.length > 0;
    await db.from("emoquad_jobs").update({
      status: okRun ? "done" : "failed",
      read: String(body.read ?? "").slice(0, 120),
      words: ws,
      err: okRun ? null : String(body.err ?? "목록 밖 응답"),
      updated_at: new Date().toISOString(),
    }).eq("id", String(body.id ?? ""));
    if (okRun) {
      await tell(db, [`🫧 감정의 사분면 — ${VIA_LABEL.worker}`, ``,
                      `추천: ${ws.join(", ")}`]);
    }
    return json({ ok: true });
  }
  ...
  if (ok === false) {
    // 하루 캡 소진 — 이게 바로 맥미니가 필요한 상황이다
    const id = await queueJob(db, text, `일일 캡 ${DAILY_CAP}건 소진`);
    if (id) return json({ queued: true, id });
    return json({ error: "오늘 무료 사용량을 다 썼습니다. 내일 다시 시도해 주세요." }, 429);
  }
  ...
  let lastErr = "";
  // gemini_models 는 싼 순서로 적어 둔다(현재 gemini-3.1-flash-lite 가 맨 앞).
  // 앞 모델이 실패할 때만 다음으로 넘어가므로 평소엔 가장 싼 모델만 쓴다.
  for (const model of models) {           // 모델 × 키 2중 폴백
    for (const key of keys) {
      try {
        ...
        const read = String(parsed?.read ?? "").slice(0, 120);
        await tell(db, [
          `🫧 감정의 사분면 — ${VIA_LABEL.edge} (${model})`,
          ``, `입력: ${text}`, `읽음: ${read || "-"}`, `추천: ${words.join(", ")}`,
        ]);
        return json({ read, words, model });
      } catch (e) {
        lastErr = String(e);
      }
    }
  }
  // 여기까지 왔다 = 제미나이가 전부 실패(대개 무료 한도 소진). 맥미니로 넘긴다.
  const id = await queueJob(db, text, lastErr);
  if (id) return json({ queued: true, id });
